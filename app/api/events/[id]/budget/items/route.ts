...
export async function GET(
  ...
  try {
    ...
    console.error(
      "Get budget items failed:",
      error,
    );

    return NextResponse.json(
      { error: "Unable to load budget items." },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    if (!budget) {
      return NextResponse.json(
        {
          error:
            "Budget not found. Create a budget first.",
        },
        { status: 404 },
      );
    }
    ...
    const item =
      await db.orm.public.BudgetItem.create({
        budgetId: budget.id,
        category,
        description,
    ...
    console.error(
      "Create budget item failed:",
      error,
    );

    return NextResponse.json(
      { error: "Unable to create budget item." },
      { status: 500 },
    );
  }
}
